...
后台面板底部还有「⇄ 翻译工作区」：上传 TXT 或 EPUB，浏览器按正文边界切块、
逐块调翻译 API、拼回导出。**它和这个目录里的后端毫无关系** —— 请求由浏览器
直发中转站，API key 只存在本机 localStorage。所以 Worker 不用为它加任何接口，
也没有流量成本。
...
更慢也更贵。单个段落超过 1300 字时，再尽量从句末、换行或空格处切；找不到边界
才硬切，避免无空行长文变成一个几千字的大块。

**EPUB 的处理方式：** EPUB 本质是 ZIP。前端用固定版本、随站点一起托管的
JSZip 3.10.1 解包，只解析 `.xhtml` / `.html` 文档里的可见文本节点：

- 翻译正文、章节标题与导航页可见目录文字
- 不翻译脚本、样式、SVG/MathML、`pre`/`code`、链接属性、图片 `alt`、OPF 元数据
- 图片、字体、CSS、OPF、NCX 等非正文文件原样保留
- 文本节点用私用区标记 `MO_TX_n` 保存边界，模型必须原样保留。标记缺失、乱序
  或某节点译文为空时，该块直接失败并禁止导出，避免把译文写错 `<p>` / `<span>`
- EPUB 必须全部块成功才允许导出；TXT 可以把未完成块用原文加「未翻译」标记占位
- 重打包时 `mimetype` 保持为 ZIP 第一项且不压缩，否则部分阅读器拒绝打开
- 导出名为 `原名.zh.epub`，图片、CSS、目录与元数据仍在

JSZip 文件在 `vendor/jszip.min.js`，不依赖运行时第三方 CDN。`test_selectors.mjs`
固定检查版本、许可证和 SHA256，防止误下载成错误页面或被无意替换。
...
**一条资源可以挂多个位置**，走 `placements` 字段，格式
`novel:download,novel:jp,manga:kr`：逗号分隔，冒号后是小分区（可省略表示只落在
该分区的「全部」里），**第一个是主归属**，决定卡片默认显示哪个标签。上限
`PLACEMENT_MAX = 8`。

为什么用一个扁平字符串而不是几列或 JSON：一条资源能挂任意多个位置，列数固定的
表存不下；存 JSON 又要额外防注入和做形状校验。扁平串好校验、出问题时肉眼能读。

**去重键是完整的「分区:小分区」对，不是分区本身。** 同一分区挂多个小分区
（小说/下载 + 小说/日轻）是真实需求 —— 一个网盘包既算下载资源又算日轻作品。
早先按分区去重，把同区的第二个小分区吃掉了，那是错的。

只有两种情况会被丢：完全相同的一对（取第一个）；同区「留空」与具体小分区并存时
丢掉留空那条 —— 留空意味着「该区全部」，已被具体那条涵盖，两个都留会让条目在该区
列表里出现两次。前端在这两种情况下会提示一句。
...
同区多小分区那组盯的是用户的原话「下载、日轻」：同区两行不当成重复、两个小分区
都存下来、该区的两个小分区页里都能找到、该区「全部」里只出现一次、卡片标签跟着
当前看的小分区走（在日轻页显示「日轻」、「也在 下载」）。
重复位置那组：完全相同的一对才提示并去重、同区「不指定」与具体小分区并存时提示
会丢掉前者。
