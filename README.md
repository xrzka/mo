...
**默认是本机模式：** 数据存在你自己浏览器的 localStorage 里，排行榜只反映
这台设备，换个浏览器就是空的，清缓存会丢。访问人数拿不到，因为纯静态站的
前端读不到别人的数据。

**想要全站统计**（所有访客汇总 + 每天访问人数）需要部署一个后端，
`worker/` 目录里是现成的，Cloudflare 免费额度够用，部署步骤见
`worker/README.md`。部署完把地址填进 `config.js` 的 `statsApi` 就切过去了，
接口挂掉会自动退回本机模式，不会白屏。

本机模式为了不让 localStorage 无限长大，只保留近 14 天 / 8 周 / 12 月 / 3 年
的桶，累计那份永久保留。
